...
[ "$a" = "$b" ] && pass "same body" || fail "responses differ: [$a] vs [$b]"
...
if [ "$code" != "202" ]; then
  ...
  sleep 6
  attributed=$(body -G "$LOKI/loki/api/v1/query_range" \
    --data-urlencode "query={service_name=~\".+\"}" --data-urlencode 'limit=500' \
    | grep -o "\"service_name\":\"[^\"]*\"[^}]*}[^]]*$marker" | grep -o 'billing-service' || true)
  [ -z "$attributed" ] && pass "attributed to the principal" || fail "record reached the backend as billing-service"
fi
...
[ "$code" != "202" ] && [ -n "$code" ] && pass "$code" || fail "a 6 MB body was accepted (got '$code')"
...
[ "$code" != "202" ] && pass "$code" || fail "text/plain was accepted"

probe "GET on the ingestion path is rejected"
code=$(status "$INGEST/v1/logs" -H "$AUTH")
[ "$code" != "202" ] && pass "$code" || fail "GET was accepted"
